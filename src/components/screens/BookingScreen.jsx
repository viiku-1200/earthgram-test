...
const BookingScreen = ({ isDarkMode, onClose }) => {
  ...
  const initialStep = (navigateMode === 'instant' && !subCategory) ? 'service-picker' : 
  ...
  // If no provider and no category (History view)
  if (!provider && !category && step !== 'mode-select' && step !== 'service-picker' && step !== 'radar' && step !== 'tracking') {
              ...
              <button key={tab} onClick={() => setActiveHistoryTab(tab)}
                className={`flex-1 py-2 rounded-lg text-xs font-bold capitalize transition-all ${
                  activeHistoryTab === tab 
                    ? (isDarkMode ? 'bg-slate-800 text-indigo-400 shadow-sm' : 'bg-white text-gray-900 shadow-sm') 
                    : 'text-gray-500'
                }`}>
                {tab}
              </button>
              ...
              <div key={booking.id} className={`p-4 rounded-2xl border shadow-premium card-lift animate-fade-in ${
                ...
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-bold">{booking.provider}</h3>
                    <p className={`text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{booking.service}</p>
                  </div>
  ...
  if (step === 'success') {
    ...
    return (
      <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center p-8 animate-slide-up ${
        ...
        <h2 className="text-2xl font-black mb-2 text-center">Service Completed!</h2>
        <p className="text-sm text-center mb-6 text-gray-500 text-center">We hope you enjoyed the experience</p>
        ...
        <button onClick={onClose} className="w-full mt-8 bg-black text-white py-4 rounded-2xl font-black text-sm active:scale-95 transition-all">Back to Home</button>
      </div>
    );
  }
  ...
  return (
    <div className={`absolute inset-0 z-50 flex flex-col animate-slide-up transition-all duration-500 ${
      ...
      <div className={`absolute bottom-0 left-0 right-0 border-t px-5 py-3 pb-6 ${isDarkMode ? 'bg-slate-900/90 border-slate-800 backdrop-blur-xl' : 'glass border-gray-100/50'}`}>
        <button onClick={() => selectedSlot && setStep('success')} disabled={!selectedSlot}
